...
def main():
    ...
    # Calculate pixel-to-centimeter ratios for width and height
    ratio_width = frame_width / width
    ratio_height = frame_height / height

    # Calculate the average of the two ratios
    average_ratio = (ratio_width + ratio_height) / 2
    # Convert the radius from centimeters to pixels and round it
    radius_pixels = round(distance_threshold * average_ratio)
    print(radius_pixels)
    while True:
        ...
        #################################################################
        debug_image = draw_tags(debug_image, tags, elapsed_time, distance_threshold, radius_pixels)
...
def draw_tags(
    ...
    if len(tags) == 2:
        ...
        cv.putText(image, angle_text, (10, 60), cv.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 0), 2, cv.LINE_AA)
        ...
        if distance_centimeters > threshold_distance:
            # draw a circle around the target
            cv.circle(image, (int(target_center_x), int(target_center_y)), radius_pixels, (0, 0, 255), 2)
        else:
            # draw a circle around the target
            cv.circle(image, (int(target_center_x), int(target_center_y)), radius_pixels, (0, 255, 0), 2)
            cv.putText(image, 'TARGET REACHED!', (10,90),
